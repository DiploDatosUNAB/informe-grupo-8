...
```{r prueba_right_join, include=FALSE}
...
```{r ls_genvar_estdesc_estdisticas_loop, eval=TRUE, warning=FALSE, include=TRUE}
...
# masculino <- two_abs_age_lga("M")
# femenino <- two_abs_age_lga("F")

data_masc <- data_per %>% 
  filter(sexo == "M") %>% 
  group_by(sexo, grupo_etario) %>% 
  summarise(n = n()) %>% 
  ggplot(aes(x = -n, y = factor(grupo_etario), fill = sexo)) +
  geom_col(fill="chocolate")
