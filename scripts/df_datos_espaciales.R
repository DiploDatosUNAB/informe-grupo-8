...
print("Se observa que la gran mayoría de personas se aplica la vacuna en su lugar de residencia, salvo pocas excepciones. Es dificil de visualizar cuantas personas se vacunan fuera de su jurisdiccion, por ello a continuacion se presenta un gráfico.")

# {r jurisdicciones_poblacion}

## Armar una tabla con la caracterizacion de la poblacion del registro respecto 
## a la cantidad de personas segun al lugar de residencia y lugar de la ultima aplicacion.
...
### diferentes de identificacion y sufijo porque ambos usan el mismo estadistico n.
...
## La tabla anterior aporta escasa informacion entonces vamos por el detalle extra:
### primero, cantidad de personas *residentes* de cada jurisdiccion que se aplican en *la misma* jurisdiccion.
jresidentes <- data_reg %>% 
  ...
  arrange(jurisdiccion_residencia) %>% 
  rename(jurisdiccion = jurisdiccion_residencia)
### segundo, cantidad de personas *aplicadas* en cada jurisdiccion que residen en otra jurisdiccion.
